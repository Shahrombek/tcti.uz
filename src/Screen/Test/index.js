...
const Test = ({ language, setUserName }) => {
  ...
  const [changeUniversity, setChangeUniversity] = useState([]);
  console.log(changeUniversity);
  useEffect(() => {
    if (
      typeLang === "rus" &&
      typeFaculty === "morning" &&
      ...
      setChangeFaculty([
        "Mehnat muhofazasi va yangi materiallar texnologiyasi (tarmoqlar boyicha)",
        "Neftь va neftь-gazni qayta ishlash texnologiyasi",
        "Neftь va gaz quvurlari, baza va omborlarini qurish va ulardan foydalanish",
        "Transport logistikasi (avtomobil transporti)",
        "Funksional ovqatlanish va bolalar maxsulotlari texnologiyasi",
        ...
        "Texnologik mashinalar va jihozlar (tarmoqlar boʼyicha)",
        "Materialshunoslik va yangi materiallar texnologiyasi (tarmoqlar boʼyicha)",
        "Kimyoviy texnologiya (qurilish materiallar)",
        "Neftь gazkimyo sanoati texnologiyasi",
        "Mexatronika va robototexnika",
        "Mashinasozlik texnologiyasi, mashinasozlik ishlab chiqarishini jihozlash va avtomatlashtirish",
        "Oziq-ovqat texnologiyasi (mahsulot turlari boʼyicha)",
      ]);
    } else if (
      typeLang === "rus" &&
      typeFaculty === "evening" &&
      ...
      setChangeFaculty([
        ...
        "Vinochilik texnologiyasi, bijgʼish mahsulotlari va alkogolsiz ichimliklar texnologiyasi",
        "Texnologik mashinalar va jihozlar (tarmoqlar boʼyicha)",
        "Kimyoviy texnologiya (ishlab chiqarish turlari boʼyicha)",
      ...
      typeLang === "rus" &&
      typeFaculty === "morning" &&
      typeEducation === "magister"
    ) {
      setChangeFaculty([
        "Neft va gazni qayta ishlash va kimyoviy texnologiyasi",
        "Neftь va gaz quvurlari, baza va omborlarini qurish va ulardan foydalanish|Neftь va gaz quvurlari, baza va omborlarini qurish va ulardan foydalanish",
        "Oziq-ovqat mahsulotlarini ishlab chiqarish va qayta ishlash texnologiyasi (mahsulot turlari boʼyicha)",
        "Xizmatlar sohasi (faoliyat turlari va yoʼnalishlari boʼyicha)",
        "Oziq-ovqat sanoati mashina va agregatlari",
        "Kimyoviy texnologiya jarayonlari va apparatlari (ishlab chiqarish turi boʼyicha)",
      ]);
    } else if (
      typeLang === "eng" &&
      typeFaculty === "morning" &&
      ...
      setChangeFaculty([
        "Oziq-ovqat xavfsizligi va sifati|Oziq-ovqat xavfsizligi va sifati",
        "Latviya",
        "Yogʼochga ishlov berish texnologiyasi va yogʼochshunoslik|Yogʼochga ishlov berish texnologiyasi va yogʼochshunoslik",
      ]);
    } else {
      setChangeFaculty([]);
    }
  ...
  const selectUniversity = (title) => {
    changeUniversity.map((item) => {
      if (item.title === title) {
        item.select = true;
        setFaculties(item.faculty);
      } else {
        item.select = false;
      }
    });
  ...
  useEffect(() => {
    ...
    typeEducation === UniversityData[0].category
      ? setChangeUniversity(UniversityData[0].body)
      : setChangeUniversity(UniversityData[1].body);
  }, [typeEducation]);
            ...
            <div className={"form-col-1"}>
              ...
              >
                <Select onChange={(e) => setTypeFaculty(e)}>
                  <Select.Option value="morning">
                    {facultyName1[language]}
                  </Select.Option>
                  <Select.Option value="evening">
                    {facultyName2[language]}
                  </Select.Option>
                </Select>
              </Form.Item>
              ...
              <Form.Item
                rules={[
                  {
                    required: true,
                    message: university[language],
                  },
                ]}
                name={"university"}
                label={university[language]}
              >
                {changeUniversity.length >= 0 ? (
                  <Select onChange={(e) => selectUniversity(e)}>
                    {changeUniversity.map((item) => (
                  ...
                  <p>{/* {'Kechirasiz bu yo`nalishda fakultetlar yo`q'} */}</p>
                )}
              </Form.Item>
              {/* //////////////////// */}
